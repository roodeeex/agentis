...
import { useMyCreations, type Order } from '@/lib/hooks/useMyCreations'
import { OrderStatus } from '@/lib/hooks/useMyOrders'
...
import { useAcceptOrderRequest } from '@/lib/hooks/useAcceptOrderRequest'
...
export default function MyCreationsPage() {
  ...
  const { orders, isLoading, error, refetch } = useMyCreations()
  const { updateOrderStatus, isUpdating } = useUpdateOrderStatus()
  const { acceptOrderRequest, isAccepting } = useAcceptOrderRequest()
  const [fees, setFees] = useState<{ disputeOrderEvaluationFee: string, disputeReviewFee: string }>({ disputeOrderEvaluationFee: '0', disputeReviewFee: '0' })
    ...
    return (
      <div className="min-h-screen bg-bg-primary pt-40 pb-12">
        <div className="max-w-4xl mx-auto px-4 text-center">
          ...
          <p className="text-text-secondary">Please connect your wallet to view your creations' orders.</p>
        </div>
      </div>
    )
  }
      ...
      <div className="max-w-5xl mx-auto px-4">
        ...
        >
          ...
          <p className="text-xl text-text-secondary max-w-2xl mx-auto">
            Manage and track the orders for the services you have created.
          </p>
        </motion.div>

        <div className="bg-white rounded-xl p-8 border border-border-primary">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-bold text-text-primary">Incoming Orders</h2>
          ...
          {isLoading && (
            <div className="text-center py-8">
              ...
              <p className="mt-4 text-text-secondary">Fetching incoming orders...</p>
            </div>
          )}
          ...
          {!isLoading && !error && orders.length === 0 && (
            <div className="text-center py-8">
              ...
              <p className="text-text-secondary">You haven't received any orders for your services yet.</p>
            </div>
          )}
                  ...
                  {orders.map((order: Order) => {
                    ...
                    return (
                      <tr key={order.id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{order.buyer}</td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                          {order.status === OrderStatus.Requested && (
                            <>
                              <button 
                                onClick={() => acceptOrderRequest(order.id, fees.disputeOrderEvaluationFee, fees.disputeReviewFee)}
